...
The current baseline is `0.5.1`: suitable for self-use with accepted static
`flow-row` / `flow-stack` pagination coverage plus follow-up editor/UX
hardening, but not yet a general-user `v1`.
