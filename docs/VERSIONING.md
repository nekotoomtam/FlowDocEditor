...
plus FlowDoc Draft Editor Island follow-up hardening for page-boundary local
height handoff, split-paragraph outer-spacing transfer, selection/clipboard
island ownership, re-enter probes, and explicit remaining scroll-anchoring and
structural split latency risks,
plus structural Enter/Backspace release-gate hardening for page-break-adjacent
plain paragraph splits, immediate merge-back behavior, stale pagination settle
...
runtime behavior,
plus FlowDoc Draft Editor Island paragraph smoothness instrumentation and
targeted hot-path hardening: draft-measure breakdowns, draft island scope and
cadence telemetry, repeated-grapheme width reuse, text-changing parent-sync
